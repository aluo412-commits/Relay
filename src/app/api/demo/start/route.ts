...
export async function POST() {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({ data: { projectId: project.id, recipientId: alex.id, kind: "connector", text: "Controller wiring affects autonomous testing — review the dependency.", importance: "important", fromName: "Relay", boardName: board.name, taskName: "Run full autonomous test" } });
