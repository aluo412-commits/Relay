...
// POST /api/board  { name, deadline?, color?, summary? }  -> create a new workstream in the active workspace.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await prisma.board.count({ where: { projectId: ctx.project.id } });
